...
#  argParser.add_argument('--usecache', dest='usecache', action='store_const', const=True, default=False, help='...')
  ...
  if args.start_step <= 7 and 7 <= args.last_step:
    ...
    #TODO: add make catchments 3d here
  ...
  if args.start_step <= 9 and 9 <= args.last_step:
    ...
    #data_bbox = "1674321.1,501746.5,1674521.8,501917.9" #test case
    ...
    if not os.path.exists(grown_catchments_gpkg_filename_with_path):
      ...
      cmd9 = "{} -cp {} -Djava.util.logging.config.file={} -Xms2g ca.bc.gov.catchment.scripts.GrowCatchmentsNicholson -waterFile {} -waterTable {} -tinEdgesFile {} -tinEdgesTable {} -o {} -outTable {} {}".format(settings.get("java_path"), settings.get("java_classpath"), settings.get("log_properties_file"), water_feature_segmented_filename_with_path, WATER_FEATURES_TABLE, tin_gpkg_filename_with_path, TIN_EDGES_TABLE, grown_catchments_gpkg_filename_with_path, CATCHMENT_LINES_TABLE, bbox)
  ...
  if args.start_step <= 10 and 10 <= args.last_step:
    ...
    cmd6 = "{} -cp {} ca.bc.gov.catchment.scripts.SmoothGeometries -i {} -inTable {} -o {} -outTable {}".format(settings.get("java_path"), settings.get("java_classpath"), grown_catchments_gpkg_filename_with_path, CATCHMENT_LINES_TABLE, catchments_smoothed_gpkg_filename_with_path, CATCHMENT_LINES_TABLE)
    resp = call(cmd6.split())
    if resp != 0:
      print("Failure.  Pipeline execution stopped early.")
      exit(1);
