...
public class WaterAwareCatchmentRouter {
	...
	public LineString reroute(LineString route, Coordinate oldCoord, Coordinate newCoord, int freedom) throws IOException, RouteException {
		...
		for (int i = 0; i < route.getNumPoints(); i++) {
			...
			else if (distance <= freedom && isCoordinateMovable(i, route) && !isEndpoint) {
	 ...
	 * @throws RouteException
	 */
	public SimpleFeature reroute(SimpleFeature feature, Coordinate oldCoord, Coordinate newCoord, int freedom) throws IOException, RouteException {
		LineString route = (LineString)feature.getDefaultGeometry();
		LineString newRoute = reroute(route, oldCoord, newCoord, freedom);
	 ...
	 * @throws RouteException
	 */
	public List<LineString> reroute(List<LineString> routes, Coordinate oldJunction, Coordinate newJunction, int freedom) throws IOException, RouteException {
		List<LineString> results = new ArrayList<LineString>();
		for (LineString route : routes) {
			LineString newRoute = reroute(route, oldJunction, newJunction, freedom);
	...
	public List<SimpleFeature> rerouteFeatures(List<SimpleFeature> routes, Coordinate oldJunction, Coordinate newJunction, int freedom) throws IOException, RouteException {
		...
		for (SimpleFeature route : routes) {
			SimpleFeature newRoute = reroute(route, oldJunction, newJunction, freedom);
			results.add(newRoute);
		}
		return results;
	}
