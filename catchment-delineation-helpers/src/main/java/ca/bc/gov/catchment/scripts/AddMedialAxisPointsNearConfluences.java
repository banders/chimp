...
public class AddMedialAxisPointsNearConfluences extends CLItoAlgorithmBridge {
...
public static void main(String[] args) {
		...
		List<Coordinate> confluenceCoords = null;
		try {
			System.out.println("Identifying confluences...");
			confluenceCoords = water.getConfluences();
			System.out.println(" - Found "+confluenceCoords.size()+" confluences");
		} 
		catch (IOException e) {
		...
		System.out.println("Processing confluence coords");
		for (Coordinate confluenceCoord : confluenceCoords) {
			
			List<SimpleFeature> touchingWater = null;
			try {
				touchingWater = water.getTouchingWater(confluenceCoord);
			}
			catch (IOException e) {
				System.out.println("unable to find water features touching confluence");
				continue;
			}
			if (touchingWater.size() < 3) {
				System.out.println("invalid confluence.  touches "+touchingWater.size() + " water features.");
				continue;
			}
			
			System.out.println("valid confluence");
			
			//copy the first water feature so it also appears at the end of the list
			touchingWater.add(touchingWater.get(0));
			
			Coordinate prevWaterSecondCoord = null;
			for (SimpleFeature waterFeature : touchingWater) {
				Geometry waterGeom = (Geometry)waterFeature.getDefaultGeometry();
				Coordinate[] coords = waterGeom.getCoordinates();
				
				//get the second coordinate of the water feature... the one adjacent to the confluence.  
				//(This may be the last coordinate of the feature, or it may be somewhere in the middle)
				Coordinate waterSecondCoord = SpatialUtils.getCoordAtIndex(coords, confluenceCoord, 1);
				
				if (prevWaterSecondCoord != null) {
					
					//create a new coordinate which is immediately between the "second coordinate" of each of 
					//the two adjacent water lines
					Coordinate medialCoord = new Coordinate(
							(waterSecondCoord.x + prevWaterSecondCoord.x) / 2,
							(waterSecondCoord.y + prevWaterSecondCoord.y) / 2
							);
					outCoords.add(medialCoord);					
				}				
				prevWaterSecondCoord = waterSecondCoord;
			}
