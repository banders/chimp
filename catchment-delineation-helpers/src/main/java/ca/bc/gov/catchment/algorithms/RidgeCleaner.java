...
public class RidgeCleaner {
	...
	private DefaultFeatureCollection truncateLoopbackLines(SimpleFeatureCollection fc) {
		...
		while(it.hasNext()) {
			...
			LineString truncated = truncateAtLoopback(original, log);
	...
	*/
	...
	public static LineString truncateAtLoopback(LineString ridge, boolean log) {
		int windowSize = 6;
		...
		if (ridge.getNumPoints() > windowSize) {
			...
			for (Coordinate c: ridge.getCoordinates()) {
				...
				if (prev != null) {
					...
					if (recentSegments.size() == windowSize) {
						double angle1 = avgAngle(recentSegments, oldStartIndex, oldEndIndex, log);
						double angle2 = avgAngle(recentSegments, newStartIndex, newEndIndex, log);
	...
	private static double avgAngle(List<LineString> segments, int startIndex, int endIndex, boolean log) {
